...
**BepInEx, looked after for you.** BepInEx is the loader every mod runs under, and until now BakaLoader said nothing about it at all. There is a row above the mod table now that says whether it is installed, which version you have, and whether BakaLoader is keeping it current. It is the same row on every server, because it is the same BepInEx: servers set up from one install share one copy of it through links on disk, which is also why a write waits until every one of them has stopped. The new switch sits in Upkeep right under **Update mods at scheduled restarts**, and BakaLoader asks once whether you want it, on your first Start press and on the Hearth for a server that starts on its own. Nothing is looked after until you have answered yes, and closing the question is not an answer. With a yes, a missing loader is put in before the server starts, a newer pack goes in during a restart once it has been out for three days, and pasting a mod link with no loader installed fetches BepInEx first and then finishes the install you asked for. Updating to 1.2.0 changes nothing about a BepInEx you already have. The question says it found one and what a yes would do: at the next restart BakaLoader brings it into its care, and if it already is the current pack that means writing a note beside it and nothing else. Otherwise the core and the loader files are replaced, with every replaced file kept in `BepInEx\.bakaloader-bepinex-backups` and the copy of your own original kept for good; your `BepInEx.cfg`, mods, mod configs and patchers stay as they are, and so does your `doorstop_config.ini` unless a new pack needs a different one, in which case yours is in the backup and you are told. It never moves you to an older BepInEx without asking, it proves no file is held open before it writes a byte, and an install that Gale or r2modman drives through its own profile is never written to unless you press the button yourself and confirm. With a no, the row's own button is the only thing that ever writes, and over an install BakaLoader did not make it asks first. If a server comes up and BepInEx never writes its log, you are told, where that used to be silent. Detail: [BepInEx](https://github.com/RyanDMcAfee/ValheimBakaLoader/wiki/BepInEx).
...
- Your server's BepInEx folders when you install, update or remove mods, and the admin, ban and allow lists when you use the player actions. BepInEx itself when you install or update it from the row above the mod table, or at a restart once you have said yes to it: the loose loader files beside the server executable, the whole of `BepInEx\core`, and the pack's `BepInEx.cfg` only when there is not one already. Your plugins, patchers and mod configs are not touched, your `doorstop_config.ini` is replaced only when a new pack needs a different one, and everything that is replaced is kept in `BepInEx\.bakaloader-bepinex-backups` first. A mod BakaLoader installed from Hexium also gets a small record inside its own folder saying where it came from, which is how the app knows to leave that folder out of its updates.
